...
class ExportService {
    static func generateHTML(devices: [NetworkDevice], config: ScanConfig, findings: [DiagnosticFinding], score: Int, notes: [DeviceNote]) -> String {
        ...
        <p class="meta">\(config.cidrNotation) · \(Date().formatted()) · \(onlineCount)/\(devices.count) 在线</p>
        <div class="score-box"><p style="margin:0;color:#666">网络健康评分</p>
        <p class="score" style="margin:8px 0">\(score)/100</p></div>
        """
        ...
        html += "<h2>设备清单</h2><table><tr><th>IP</th><th>MAC</th><th>厂商</th><th>类型</th><th>延迟</th><th>状态</th><th>备注</th></tr>"
        for d in devices.sorted(by: { ($0.latencyMs ?? 9999) < ($1.latencyMs ?? 9999) }) {
            ...
            html += "<tr><td>\(d.ipAddress)</td><td>\(d.macAddress ?? "--")</td><td>\(d.vendor ?? "--")</td><td>\(d.deviceType.rawValue)</td><td>\(lat)</td><td>\(d.isOnline ? "✓" : "✗")</td><td><span class=\"note-tag\">\(noteText)</span></td></tr>"
        }
        html += "</table>"

        html += """
        <div class="footer"><p class="watermark">Generated by NetDiagnose — 免费网络健康诊断工具</p>
        <p style="font-size:10px;color:#999">81677632@qq.com</p></div></body></html>
        """
        return html
    }
    ...
    static func generateMarkdown(devices: [NetworkDevice], config: ScanConfig, findings: [DiagnosticFinding], score: Int, notes: [DeviceNote]) -> String {
        ...
        md += "**子网**: \(config.cidrNotation) · **时间**: \(Date().formatted())\n\n"
        md += "## 健康评分: \(score)/100\n\n"
        ...
        md += "## 设备清单\n\n"
        let noteMap = Dictionary(uniqueKeysWithValues: notes.map { ($0.ip, $0) })
        md += "| IP | MAC | 厂商 | 类型 | 延迟 | 状态 | 备注 |\n"
        md += "|---|---|---|---|---|---|---|\n"
        for d in devices.sorted(by: { ($0.latencyMs ?? 9999) < ($1.latencyMs ?? 9999) }) {
            let lat = d.latencyMs.map { String(format: "%.1f ms", $0) } ?? "--"
            let note = noteMap[d.ipAddress]?.label ?? ""
            md += "| \(d.ipAddress) | \(d.macAddress ?? "--") | \(d.vendor ?? "--") | \(d.deviceType.rawValue) | \(lat) | \(d.isOnline ? "✓" : "✗") | \(note) |\n"
        }
        md += "\n---\n*Generated by NetDiagnose — 81677632@qq.com*"
        return md
    }
